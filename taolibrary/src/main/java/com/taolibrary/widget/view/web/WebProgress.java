package com.taolibrary.widget.view.web;
...
public class WebProgress extends FrameLayout {
    ...
    public static final int VIEW_INVISIBLE = 3;
    ...
    private void doEnd() {
        if (TAG == VIEW_GONE && mCurrentProgress == 100f) {
            setVisibility(GONE);
            mCurrentProgress = 0f;
            this.setAlpha(1f);
        } else if (TAG == VIEW_INVISIBLE && mCurrentProgress == 100f) {
    ...
    public void setProgress(float progress) {
        ...
        if (TAG != VIEW_GONE && TAG != VIEW_INVISIBLE ) {
            startAnim(true);
        }
    }
